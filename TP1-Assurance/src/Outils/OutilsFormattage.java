...
public class OutilsFormattage {
	...
	public static boolean formatFichier( String fichier ) throws IOException {
		...
		String erreurFichier;
		
		boolean pasManque = true;
		boolean pasDoublon = true;
		boolean finAlaFin = true;

		if ( (erreurFichier = formatFichierExistance( fichier )).length() > 0 ) {
			messageErreur += erreurFichier + "\n";
			pasManque = false;
		}
		
		if ( (erreurFichier = formatFichierDoublon( fichier )).length() > 0 ) {
			messageErreur += erreurFichier + "\n";
			pasDoublon = false;
		}
		
		if ( ( erreurFichier = formatFichierFin(fichier )).length() > 0 ) {
			messageErreur += erreurFichier + "\n";
			finAlaFin = false;
		}

		if ( !pasManque || !pasDoublon || !finAlaFin ) {
		...
		return ( pasManque && pasDoublon && finAlaFin );
	}
	
	public static String formatFichierExistance( String fichier ) throws IOException {
		...
		if ( !( fichier.contains( OutilsConstante.LISTE_CLIENT ) && fichier.contains( OutilsConstante.LISTE_PLAT )
				...
				&& fichier.contains( OutilsConstante.LISTE_FIN ) ) ) {
	...
	public static String formatFichierDoublon( String fichier ) {
		String messageErreur = "";
	...
	public static String formatFichierFin( String fichier ) {
		String messageErreur = "";
		
		if ( fichier.contains( OutilsConstante.LISTE_FIN ) && !fichier.endsWith( OutilsConstante.LISTE_FIN ) ) {
			messageErreur = "\"" + OutilsConstante.LISTE_FIN + "\" n'est pas manquant, mais doit etre a la "
					+ "fin!\n";
		}
		return messageErreur;
	}
